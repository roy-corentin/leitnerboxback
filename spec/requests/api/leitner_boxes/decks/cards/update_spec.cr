...
describe Api::LeitnerBoxes::Decks::Cards::Update do
  describe "user authenticated" do
    describe "card exist" do
      it "should return the card updated" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Update.with(leitner_box.id, deck.id, card.id), card: valid_params)
    ...
    describe "card does not exist" do
      it "fails to fetch the card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Update.with(100, 100, 2), card: valid_params)
    ...
    describe "card does not belongs to current user" do
      it "fails to update the card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Update.with(100, 100, card.id), card: valid_params)
