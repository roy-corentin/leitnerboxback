...
describe Api::LeitnerBoxes::Decks::Cards::Show do
  describe "user authenticated" do
    describe "card exist" do
      it "should return the existing card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Show.with(leitner_box.id, deck.id, card.id))
    ...
    describe "card does not belongs to current user" do
      it "fails to fetch the card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Show.with(100, 100, card.id))
        response.status_code.should eq(404)
      end
    end
  end
