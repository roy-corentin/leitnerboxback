...
describe Api::LeitnerBoxes::Decks::Cards::Delete do
  describe "user authenticated" do
    describe "card exist" do
      it "should delete the existing card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Delete.with(leitner_box.id, deck.id, card.id))
    ...
    describe "card does not exist" do
      it "fails to delete the card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Delete.with(100, 100, 2))
    ...
    describe "card does not belongs to current user" do
      it "fails to delete the card" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Cards::Delete.with(100, 100, card.id))
        response.status_code.should eq(404)
      end
    end
  end
