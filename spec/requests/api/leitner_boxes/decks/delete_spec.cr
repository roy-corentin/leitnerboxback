...
describe Api::LeitnerBoxes::Decks::Delete do
  describe "user authenticated" do
    describe "deck exist" do
      it "should delete the existing deck" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Delete.with(leitner_box.id, deck.id))
    ...
    describe "deck does not exist" do
      it "fails to delete the deck" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Delete.with(100, 2))
    ...
    describe "deck does not belongs to current user" do
      it "fails to delete the deck" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Delete.with(100, deck.id))
        response.status_code.should eq(404)
      end
    end
  end
