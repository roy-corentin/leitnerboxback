...
describe Api::LeitnerBoxes::Decks::Update do
  describe "user authenticated" do
    describe "deck exist" do
      it "should return the deck updated" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Update.with(leitner_box.id, deck.id), deck: valid_params)
    ...
    describe "deck does not exist" do
      it "fails to fetch the deck" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Update.with(100, 2), deck: valid_params)
    ...
    describe "deck does not belongs to current user" do
      it "fails to update the deck" do
        ...
        response = ApiClient.auth(user).exec(Api::LeitnerBoxes::Decks::Update.with(100, deck.id), deck: valid_params)
