class FetchNextCardToStudy < Avram::Operation
  ...
  def run
    ...
    decks_to_check.each do |deck|
      if (card_to_study = deck.next_card_to_study?).nil?
        SaveDeck.update!(deck, last_review_at: Time.utc, user_id: user_id.value.not_nil!, leitner_box_id: leitner_box_id.value.not_nil!)
      else
