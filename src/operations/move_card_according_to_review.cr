class MoveCardAccordingToReview < Avram::Operation
  ...
  def move_card_to_upper_deck
    ...
    upper_deck = DeckQuery.new.leitner_box_id(leitner_box_id.value.not_nil!).level.gt(deck.level).first?
    if upper_deck
  ...
  def move_card_to_lower_deck
    card = CardQuery.find(card_id.value.not_nil!)
    deck = DeckQuery.find(deck_id.value.not_nil!)
    lower_deck = DeckQuery.new.leitner_box_id(leitner_box_id.value.not_nil!).level.desc_order.level.lt(deck.level).first?
    if lower_deck
